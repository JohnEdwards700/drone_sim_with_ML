...
#max episode steps is maximum steps it can take per training session
    ...
    def step(self, action):
        ...
        # progress_of_distance_reward = max(1,abs(total_dist)-abs(dist_to_go))
        ...
        reward -= 1  # Encourage faster reaching of the goal

        # Reward for reducing distance to the goal
        distance_reduction = self.prev_dist_to_goal - dist_to_goal
        reward += distance_reduction * 10  # Amplify reward for progress
        ...
        # Penalize for no movement or staying in the same position
        if np.allclose(current_pos, self.prev_pos, atol=0.1):  # Check for minimal movement
            reward -= 5
